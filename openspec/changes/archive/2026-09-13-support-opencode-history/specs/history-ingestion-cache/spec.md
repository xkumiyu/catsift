...
システムはreportの再生成に必要な正規化済みfieldだけをcacheへ保存し、raw prompt本文、tool引数、および不要なraw provider payloadを保存してはならない（MUST）。cacheはユーザーのlocal machine内だけで扱い、履歴内容をnetworkへ送信してはならない（MUST NOT）。この規則はCodex、ctx、およびOpenCodeのcacheへ同じように適用しなければならない（SHALL）。
